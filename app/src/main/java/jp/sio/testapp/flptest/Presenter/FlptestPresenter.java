...
import jp.sio.testapp.flptest.Service.FlpBalancedPowerAccuracyService;
...
public class FlptestPresenter {
    ...
    private FlpBalancedPowerAccuracyService flpBalancedPowerAccuracyService;
    private FlpHighAccuracyService flpHighAccuracyService;
    private FlpLowPowerService flpLowPowerService;
    ...
    private ServiceConnection serviceConnectionFlpBalancedPowerAccuracy = new ServiceConnection() {
        @Override
        public void onServiceConnected(ComponentName name, IBinder service) {
            flpBalancedPowerAccuracyService = ((FlpBalancedPowerAccuracyService.FlpBalancedPowerAccuracyService_Binder)service).getService();
        }

        @Override
        public void onServiceDisconnected(ComponentName name) {
            activity.unbindService(runService);
            flpBalancedPowerAccuracyService = null;
        }
    };

    private ServiceConnection serviceConnectionFlpHighAccuracy = new ServiceConnection() {
        @Override
        public void onServiceConnected(ComponentName componentName, IBinder service) {
            flpHighAccuracyService = ((FlpHighAccuracyService.FlpHighAccuracyService_Binder)service).getService();
        }

        @Override
        public void onServiceDisconnected(ComponentName componentName) {
            activity.unbindService(runService);
            flpHighAccuracyService = null;

        }
    };

    private ServiceConnection serviceConnectionFlpLowPower = new ServiceConnection() {
        @Override
        public void onServiceConnected(ComponentName componentName, IBinder service) {
            flpLowPowerService = ((FlpLowPowerService.FlpLowPowerService_Binder)service).getService();
        }

        @Override
        public void onServiceDisconnected(ComponentName componentName) {
            activity.unbindService(runService);
            flpLowPowerService = null;

        }
    };

    private ServiceConnection serviceConnectionFlpNoPower = new ServiceConnection() {
        @Override
        public void onServiceConnected(ComponentName componentName, IBinder service) {
            flpNoPowerService = ((FlpNoPowerService.FlpNoPowerService_Binder)service).getService();
        }

        @Override
        public void onServiceDisconnected(ComponentName componentName) {
            activity.unbindService(runService);
            flpNoPowerService = null;
        }
    };
    ...
    public void locationStart(){
        ...
        if(locationType.equals(activity.getResources().getString(R.string.locationFlpBalancedPowerAccuracy))) {
            locationserviceIntent = new Intent(activity.getApplicationContext(), FlpBalancedPowerAccuracyService.class);
